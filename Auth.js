  ...
  const lengthProps = input => {
    ...
    if (props.authLoad) {
        ...
        if (isSignup) {
            formBody = ( <form className={classes.form}  onSubmit = {onAuthSubmit}>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12}>
                  {inputCreateHandler('email',{margin: 'none',type:'email'})}
                  </Grid>
                  <Grid item xs={12}>
                  {inputCreateHandler('password',{margin: 'none', type:'password'})}
                  </Grid>
        ...
        if (!isSignup && !forgotPassword) {        
            formBody = (<form className={classes.form} onSubmit={onAuthSubmit}>
              {inputCreateHandler('email', { margin:'normal', autoFocus:true, type:'email' })}
              {inputCreateHandler('password', { margin:'normal',type:'password'})}
        ...
        if (!isSignup && forgotPassword) {
          formBody = (
            <form className={classes.form} onSubmit={onPasswordReset}>
              ...
              {inputCreateHandler('email', {
                margin: 'normal', autoFocus: true, type: 'email'}) }
